	...
	searchStocks(e) {
		...
		if (filter.length === 0) {
			...
			for (let i = 0; i < allSymbols.length; i++) {
				let splitSymbol = allSymbols[parseInt(i)].toUpperCase().split("");
				let splitFilter = filter.toUpperCase().split("");
				for (let a = 0; a < splitFilter.length; a++) {
					if (
						allSymbols[parseInt(i)].toUpperCase().indexOf(filter) > -1 &&
						splitSymbol[parseInt(a)] === splitFilter[parseInt(a)]
					) {
						if (a === 0) {
							console.log(allSymbols[parseInt(i)])
